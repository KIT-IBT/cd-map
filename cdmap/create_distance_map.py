...
def parseargs():
    ...
    parser.add_argument('--input_path_landmarks', '-l', type=str, default=None,
        help='input path for landmark file (if shape model instance, not required)')
    parser.add_argument('--ray_file', type=str, default=None, help='ray file for icosphere')
    parser.add_argument('--num_rays_x', type=int, default=None,
                        help='number of rays in x direction, overwrites num_rays')
    parser.add_argument('--num_rays_y', type=int, default=None,
                        help='number of rays in y direction, overwrites num_rays')
    parser.add_argument('--num_rays', type=int, default=49,
                        help='number of rays equals image length')
    parser.add_argument('--output_path_hit_points', '-p', type=str, default=None,
                        help='output hit points')
    parser.add_argument('--output_path_distances', '-d', type=str, default=None,
                        help='output distances from map')
    parser.add_argument('--output_path_image', '-m', type=str, default=None,
                        help='output image (png)')
    parser.add_argument('--method', type=str, default='halfsphere',
                        help='method, default half sphere')
...
if __name__ == '__main__':
    ...
    if args.input_path_landmarks is None:
        PLY_DATATYPE = "instance"
    else:
        PLY_DATATYPE = "subject"

    if (args.output_path_distances is None and
        args.output_path_image is None and
        args.output_path_hit_points is None):
        warnings.warn("No output file specified.")

    # Load mesh and create rays
    loader = distmaps.MeshLoader(path_to_file=args.input_path_ply,
                                 path_to_lms=args.input_path_landmarks,
                                 datatype=PLY_DATATYPE)
    ...
    creator = distmaps.RayCreator(method=args.method,
                                  intersector=intersector,
                                  num_rays_x=args.num_rays_x,
                                  num_rays_y=args.num_rays_y)
    ...
    if args.output_path_hit_points is not None:
    ...
    if args.output_path_distances is not None or args.output_path_image is not None:
        if args.ray_file is None:
            ...
            distances = creator.distances_from_target_points(hit_points_list,
                                                             tuples_in,
                                                             num_dimensions=1)
        if args.output_path_distances is not None:
            numpy.save(args.output_path_distances,distances)
        if args.output_path_image is not None:
